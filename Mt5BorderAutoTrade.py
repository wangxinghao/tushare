...
def close_position_if_neccessary(symbol,positions,current_price,close_point,box_high,box_low):
    ...
    df5 = Mt5Lib.get_historical_data(symbol, mt5.TIMEFRAME_M5, 3)
    ...
    for position in positions:
        ...
        if position.type == mt5.POSITION_TYPE_BUY:
            ...
            if loss_points >= close_point:
                ...
                Mt5Lib.close_position(symbol, position.ticket, mt5.ORDER_TYPE_BUY, position.volume)
                ...
                Mt5Lib.close_position(symbol, position.ticket, mt5.ORDER_TYPE_BUY, position.volume/2)
                FIVE_MIN_OPEN_FLAG,FIVE_MIN_VOLUME_FLAG,TARGET_OBJECT_PRICE_1,TARGET_OBJECT_PRICE_2 \
                    = False,False,0,0
        ...
        # 对于卖单，如果当前价格高于开仓价格4点则平仓
            ...
            if loss_points >= close_point:
                ...
                Mt5Lib.close_position(symbol, position.ticket, mt5.ORDER_TYPE_SELL, position.volume)
                FIVE_MIN_OPEN_FLAG,FIVE_MIN_VOLUME_FLAG,TARGET_OBJECT_PRICE_1,TARGET_OBJECT_PRICE_2 \
                    = False,False,0,0
                ...
                Mt5Lib.close_position(symbol, position.ticket, mt5.ORDER_TYPE_SELL, position.volume/2)
                FIVE_MIN_OPEN_FLAG,FIVE_MIN_VOLUME_FLAG,TARGET_OBJECT_PRICE_1,TARGET_OBJECT_PRICE_2 \
                    = False,False,0,0
            elif TARGET_OBJECT_PRICE_2 >0 and current_price <= TARGET_OBJECT_PRICE_2:
...
def tagetPrice(symbol,current_price,POSITION_TYPE):
    df15 = Mt5Lib.get_historical_data(symbol, mt5.TIMEFRAME_M5, 200)
...
def open_position_if_neccessary(symbol,current_price,box_high,box_low,volume):
    global FIVE_MIN_OPEN_FLAG, FIVE_MIN_VOLUME_FLAG,TARGET_OBJECT_PRICE_1,TARGET_OBJECT_PRICE_2
    df5 = Mt5Lib.get_historical_data(symbol, mt5.TIMEFRAME_M5, 4)
    ...
    first_two_tick_volume_change = df5.iloc[0]['tick_volume'] + df5.iloc[1]['tick_volume'] + weight*70 + 30
    ...
    if  current_price > box_high and current_price > m5_high:
        if not FIVE_MIN_VOLUME_FLAG and df5.iloc[-1]['high']>df5.iloc[-2]['high']:
            FIVE_MIN_OPEN_FLAG = True
            logger.info(f"价格突破箱体上沿 {box_high}, 发送买入订单")
            Mt5Lib.send_order(symbol, volume, mt5.ORDER_TYPE_BUY)
            TARGET_OBJECT_PRICE_1,TARGET_OBJECT_PRICE_2 = tagetPrice(symbol,current_price,mt5.ORDER_TYPE_BUY)
    ...
    # 跌破下沿, 做空
        ...
        if not FIVE_MIN_VOLUME_FLAG and df5.iloc[-1]['low']>df5.iloc[-2]['low']:
            FIVE_MIN_OPEN_FLAG = True
            logger.info(f"价格跌破箱体下沿 {box_low}, 发送卖出订单")
            Mt5Lib.send_order(symbol, volume, mt5.ORDER_TYPE_SELL)
            TARGET_OBJECT_PRICE_1,TARGET_OBJECT_PRICE_2 = tagetPrice(symbol,current_price,mt5.ORDER_TYPE_SELL)
...
def box_breakout_strategy(symbol, timeframe, volume, close_point):
    ...
    while True:
        ...
        try:
            ...
            hasVolumn = positions is not None and len(positions) != 0
        ...
        TradeUtils.wait_for_time(hasVolumn, current_price, box_high, box_low)
